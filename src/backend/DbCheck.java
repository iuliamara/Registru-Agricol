...
public class DbCheck {
    public static void main(String[] args) {
        String url = System.getenv().getOrDefault("SPRING_DATASOURCE_URL", "jdbc:postgresql://localhost:5432/multi_tenant_db");
        String user = System.getenv().getOrDefault("SPRING_DATASOURCE_USERNAME", "admin");
        String password = System.getenv("SPRING_DATASOURCE_PASSWORD");

        if (password == null || password.isBlank()) {
            throw new IllegalStateException("SPRING_DATASOURCE_PASSWORD is not set");
        }
